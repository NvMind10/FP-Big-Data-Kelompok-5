...
# ==========================================
# 2. PABRIK DATA SIMULASI (Mock Data Ingestion)
# ==========================================
CSV_FILENAME = "dataset_2026.csv"

# Fungsi untuk membuat data awal jika file belum ada
def inisialisasi_data_awal():
    base_time = datetime.now() - timedelta(hours=2)
    mock_senders = ["@loker_cuan_kilat", "@crypto_indonesia_bot", "@budi_safe", "@siska_hrd_palsu", "@riki_wibowo", "@halo_surabaya", "@investasi_amanah"]
    mock_groups = ["Grup Lowongan Kerja JKT", "Komunitas Pencari Cuan", "Obrolan Santai Alumni", "Info Investasi 2026", "Warga Surabaya Olx"]
    mock_texts = [
        "Tugas mudah cuma like dan subscribe youtube dapat 50rb per menit. Hubungi WA.",
    ...
    timestamps, senders, groups, texts, predictions = [], [], [], [], []
    
    # Generate 30 data awal acak
    for i in range(30):
        t = (base_time + timedelta(minutes=4 * i)).strftime("%Y-%m-%d %H:%M:%S")
        idx = np.random.randint(0, len(mock_texts))
        timestamps.append(t)
        senders.append(np.random.choice(mock_senders))
        groups.append(np.random.choice(mock_groups))
        texts.append(mock_texts[idx])
        predictions.append("🚨 SCAM" if idx in [0, 2, 4] else "✅ AMAN")
        
    df_init = pd.DataFrame({
        "timestamp": timestamps, "sender": senders, "group": groups, "text": texts, "prediction": predictions
    })
...
# ==========================================
# 3. LOGIKA OTOMATIS TAMBAH DATA (Simulasi Live Streaming)
# ==========================================
# Menambahkan 1 data baru setiap kali halaman di-refresh agar terlihat "Real-Time Streaming"
df_current = pd.read_csv(CSV_FILENAME)

# Batasi pertumbuhan data biar file tidak membengkak di komputer Anda
if len(df_current) < 150:
    live_sender = np.random.choice(["@loker_palsu_baru", "@bot_giveaway_2026", "@penipu_ulung", "@user_biasa"])
    live_group = np.random.choice(["Grup Lowongan Kerja JKT", "Crypto Cuan Group", "Arisan Online", "Info Bisnis Kilat"])
    
    # Acak apakah data baru ini scam atau aman
    is_scam = np.random.choice([True, False], p=[0.4, 0.6])
    if is_scam:
        live_text = "💥 PROMO TERBATAS! Klik link ini untuk klaim saldo DANA gratis 2jt rupiah hari ini tanpa syarat!"
        live_pred = "🚨 SCAM"
    else:
        live_text = "Permisi kak, mau tanya lowongan untuk bagian IT Support apakah masih ada?"
        live_pred = "✅ AMAN"
